...
test('skeleton suggest block contains all mail drivers', function () use ($knownDriversPath, $skeletonComposerPath) {
    KnownDriversValidator::assertSkeletonSuggestContainsAll($knownDriversPath, $skeletonComposerPath);
});

test('every mail driver follows marko slash prefix pattern', function () use ($knownDriversPath) {
